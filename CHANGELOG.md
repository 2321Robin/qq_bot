...
### Added
- Added comma-chain splitting for autonomous replies (S7-AUTO-P2-10): generated replies are deterministically split on pause punctuation into at most 3 short messages sent as a human-like burst (0.6~1.4s inter-part delay, burst fragments via raw API with silent per-part failure, final part through the full finish/retry path); prompt length guidance tightened to ~20 chars.
- Added auto-chat reply diversity (S7-AUTO-P2-09): persona prompt no longer embeds quotable example lines (they were being parroted verbatim); casual generator gained hard anti-formula rules (no sentence-final periods, punctuation minimal, emoji rare, no consecutive modal-particle openers, vary phrasing); a per-reply random style directive (`style_directive`) plus an in-memory per-group ring buffer of the last 8 autonomous replies injected into the prompt; casual temperature raised to 0.85 (`AUTO_CHAT_REPLY_TEMPERATURE`).
- Added grounded weibo hot-search intros (2026-09-19 user request): when `REPORT_AI_ENABLED` and web search are available, each hot entry (default `REPORT_HOT_MAX_ITEMS=10`, deduplicated) is sent to the report LLM with per-item search snippets and rendered as `N. 标题：一句话介绍` plus an optional 【今日小结】 overall line — both restricted to fetched snippets by a positional deterministic check with rank-order repair; no material (search off/failing/capped) or any check failure degrades to the plain numbered title template, and the LLM never blocks the report.
...
- Added autonomous group chat (S7-AUTO): `AUTO_CHAT_ENABLED` gates a four-stage pipeline over plain (non-addressed) group messages — zero-cost static prefilter, sampling (`AUTO_CHAT_SAMPLE_RATE`), strict-JSON LLM gate on the router model (fail-closed on parse errors/low confidence/gateway failures), and a casual persona generator (plain-text short replies, `max_tokens` 100) with per-group in-memory throttling (cooldown, hourly/daily caps, negative-feedback backoff that also covers the nicknamed fast-path), quota admission before and after generation, randomized pre-send delay, and `qq_bot_auto_chat_total{stage,result}` metrics plus `auto.gate`/`auto.generate` spans; config-driven persona (`PERSONA_NAME`/`PERSONA_ALIASES`/`PERSONA_PROMPT`) is also injected into the @-addressed Q&A system prompt, and fixed clarify/unavailable replies became variant pools.

- Added run-metrics collection script `scripts/collect_run_metrics.py` (S5-METRICS): fetches `/metrics` (or a saved text export), aggregates `chat_memory.sqlite3` counts and quota tables into a Pydantic-fixed daily report (`data/reports/metrics-<date>.json`, `schema_version=1`) plus a cumulative `metrics-state.json` (`first_seen` drives `uptime_days`); group/user dimensions are emitted as counts + sha256 hashes only (never raw ids); unobserved fields are marked `not_observed` instead of fabricated zeros; `--no-write` prints without touching disk. First collection landed 2026-08-03 against the copy-instance run.
- Added Python 3.11 compat guard in `download_roco_data.py` (`_extract_to`): the `filter="data"` argument of `TarFile.extractall` is used only when `tarfile.data_filter` exists, otherwise omitted — 3.11.0 raises on the kwarg itself, and the guard keeps the safe-members rejection behavior (two new tests, both states covered).
- Added order-preserving dedup of `evolution.needs_confirmation` in `compute_diff` (`dict.fromkeys` over added+modified edges): cross-file duplicated edges (the same evolution pair mirrored in two pet files) are emitted once; the 2026-08-02 refresh's 208 entries reduce to 105 unique (two new tests).
- Added first live benchmark run (2026-08-03, `glm-4-flash` free model): honest report with failing gates (`EVAL_EXIT=1`) — see README evaluation table; human review sheet for the 45 sampled failures is awaiting deployer labels.
...
- Changed life-report and game-calendar list rendering from `-`/`·` bullets to `1.` numbering (2026-09-19 user request): QQ plain-text messages render bare bullets as literal characters; the news polish prompt now emits numbered entries too, and the deterministic checks are bullet-style agnostic as before.
- Removed the 【寄语】 sign-off from report LLM polish (2026-09-19 user decision): the polish model only ever sees titles, so any editorial comment lacks factual grounding; a stray 寄语/小结 line in model output is stripped instead of failing the check, and news output never carries comment lines (the hot section's grounded 小结 is the only remaining editorial line).
- Split `REPORT_HOTLIST_MAX_ITEMS` into `REPORT_HOT_MAX_ITEMS` (default 10, weibo hot search) and `REPORT_HEH_MAX_ITEMS` (default 3, 小黑盒 hot posts); the old env name is ignored (`extra="ignore"`), and duplicate hot entries are dropped before truncation (whitespace/punctuation-insensitive match).
