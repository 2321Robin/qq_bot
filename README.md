...
| 手动 AI 早报 | `/AI早报` | 在当前群主动触发一次 AI 早报（与定时任务同一组装管线） |
...
- **+1 跟发（二期）：** ≥2 人发一模一样的消息时，机器人跟发一条一样的（受敏感词/退避/限额护栏约束，短期内不重复跟发同一条）。
- **冷场判定收紧（二期）：** 仅当自己上一条发言具有提问性质（问号或吗/呢/吧/嘛结尾）且 180 秒无人接话时才补话；补话不重复自己最近的句式。
- **调优记录（二期）：** `AUTO_CHAT_TUNE_LOG_ENABLED=true` 时把每次自主回复的上下文与回复写入 `data/auto_chat_tune.jsonl`，便于复盘标注好坏。
- **防伪影与格式（二期）：** 上下文渲染匿名化（用户A/B/C，防 QQ 号被复读）；自主回复禁感叹号、表情与文字之间不夹标点；含长数字伪影的回复整条放弃。
- **逗号长句拆分（二期）：** 生成的逗号长句在发送前确定性拆成多条短消息连发（最多 3 条，碎片间 0.6~1.4 秒打字间隔），符合真人群聊习惯；最后一条走完整重试链路。
- **回复多样性（二期）：** 人设 prompt 禁止内嵌示例台词；生成硬规则（句末不加句号、标点能省则省、表情稀缺化、语气词开头不得连用）+ 每条随机风格指令 + 最近发言注入（避免复读自己），生成温度默认 0.85（`AUTO_CHAT_REPLY_TEMPERATURE`）。
- **上下文时效窗（二期）：** 决策门/生成只看最近 `AUTO_CHAT_CONTEXT_MAX_AGE_MINUTES`（默认 30 分钟）的消息，隔天旧话题不再污染回复；生成指令强制先回应最新消息本身。
- **人设：** `PERSONA_NAME=鸡煲`（别名 `煲仔,煲煲`）已上线；默认人设为爱玩梗、自嘲的群友风格。
