...
- NapCat shell from `C:\Users\Robin\Documents\NapCatQQ\NapCat.44498.Shell` using `NapCatWinBootMain.exe ACCOUNT_PLACEHOLDER`.

It should also open the NapCat WebUI at `http://127.0.0.1:6099/webui?token=TOKEN_PLACEHOLDER` and report whether NapCat has established a connection to the backend on port `8081`.
